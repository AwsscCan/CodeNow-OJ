...
type EditorTheme = "light" | "dark" | "girl";
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (saved === "light" || saved === "dark" || saved === "girl") setEditorTheme(saved);
  ...
  useEffect(() => {
    ...
    if (!saved) return;
    try {
      const position = JSON.parse(saved) as Partial<MascotPosition>;
      if (typeof position.x === "number" && typeof position.y === "number") setMascotPosition(position as MascotPosition);
    } catch { /* ignore malformed local state */ }
  }, []);

  useEffect(() => {
    if (mascotPosition) localStorage.setItem("codenow-mascot-position", JSON.stringify(mascotPosition));
  }, [mascotPosition]);
  ...
  const directChildFolders = selectedFolder === "全部题目" ? [] : orderedFolders.filter((folder) => folderParent(folder) === selectedFolder);
  ...
  function cycleMascot() {
    setMascotMessage((value) => (value + 1) % mascotStates.length);
  }
  ...
  function explainLockedProblemId(kind: "built-in" | "acwing") {
    toast(kind === "built-in" ? "内置题目编号不可修改，可先复制题面导入为自己的题目" : "AcWing 导入题属于来源题库，不改原编号；可复制为自定义题后改号");
  }
  ...
  async function generateMoreTests() {
    ...
    try {
      ...
      toast(report ? `${report.partial ? "已快速生成可用数据" : "生成完成"}：${fresh.length} 个新测试点，含 ${report.performanceCount} 个性能点、${report.adversarialCount} 个强反例` : `AI 已补充 ${fresh.length} 个不重复测试点`);
    } catch (error) {
      toast(error instanceof DOMException && error.name === "AbortError" ? "生成超过 45 秒已停止，请改用较少测试点后重试" : error instanceof Error ? error.message : "AI 测试点生成失败");
    } finally {
      ...
      {pageView === "library" ? <section className="library-page">
        ...
        <div className="library-page-body">
          ...
          <main className="library-catalog">
            ...
            {!includeSubfolders && directChildFolders.length > 0 && <section className="catalog-subfolders" aria-label="子文件夹"><header><b>子文件夹</b><span>{directChildFolders.length} 个</span></header><div>{directChildFolders.map((folder) => { const problemCount = archives.filter((item) => folderContains(item.folder, folder)).length + acwingCourse.filter((item) => folderContains(item.folder, folder)).length + (folderContains("默认题库", folder) ? 1 : 0); const childCount = orderedFolders.filter((item) => folderParent(item) === folder).length; return <button key={folder} onClick={() => setSelectedFolder(folder)} title={`进入 ${folder}`}><span className="subfolder-icon">▱</span><span><b>{folderName(folder)}</b><small>{problemCount} 道题目{childCount > 0 ? ` · ${childCount} 个子文件夹` : ""}</small></span><i>进入 ›</i></button>; })}</div></section>}
            ...
            <div className="catalog-list">
              {catalogItems.map((entry) => {
                if (entry.kind === "built-in") return <article className="catalog-row built-in" key={`${entry.kind}-${entry.id}`}><div className="catalog-problem-link"><button className="catalog-id-edit locked" title="内置题目编号不可修改" aria-label={`题号 ${initialProblem.id} 不可修改`} onClick={() => explainLockedProblemId("built-in")}><code>{initialProblem.id}</code></button><button className="catalog-title-open" onClick={() => { setProblem(initialProblem); setCode(starterCode); setResults([]); setCompilerDiagnostic(""); setPageView("workspace"); }}><span><b>{initialProblem.title}</b><small>经典入门题 · 内置题目</small></span></button></div><span className="difficulty beginner">{initialProblem.difficulty}</span><span>{initialProblem.samples.length} 个</span><span>默认题库</span><i onClick={() => { setProblem(initialProblem); setCode(starterCode); setResults([]); setCompilerDiagnostic(""); setPageView("workspace"); }}>进入做题 →</i></article>;
                if (entry.kind === "acwing") { const item = entry.item; return <article className="catalog-row external-problem" key={`${entry.kind}-${entry.id}`}><div className="catalog-problem-link"><button className="catalog-id-edit locked" title="来源题库编号不可直接修改" aria-label={`题号 ${item.id} 不可直接修改`} onClick={() => explainLockedProblemId("acwing")}><code>{item.id}</code></button><button className="catalog-title-open" onClick={() => openBundledProblem(item)}><span><b>{item.title}</b><small>{item.extractionStatus === "complete" ? "题面已自动提取" : "题面需结合来源核对"} · 博客园来源</small></span></button></div><span className="difficulty normal">{item.difficulty}</span><span>{item.samples.length} 个</span><span title={item.folder}>{folderName(item.folder)}</span><div className="row-actions"><a href={item.sourceUrl} target="_blank" rel="noreferrer">来源</a><i onClick={() => openBundledProblem(item)}>进入 →</i></div></article>; }
                const item = entry.item; return <article className="catalog-row" key={`${entry.kind}-${entry.id}`}><div className="catalog-problem-link"><button className="catalog-id-edit editable" title="点击修改题号" aria-label={`修改题号 ${item.problem.id}`} onClick={() => beginRenameProblem(item.problem.id)}><code>{item.problem.id}</code></button><button className="catalog-title-open" onClick={() => openArchivedProblem(item)}><span><b>{item.problem.title}</b><small>{new Date(item.archivedAt).toLocaleDateString("zh-CN")} 归档</small></span></button></div><span className={`difficulty ${item.problem.difficulty === "提高" ? "advanced" : item.problem.difficulty === "普及" ? "normal" : "beginner"}`}>{item.problem.difficulty}</span><span>{item.problem.samples.length} 个</span><select aria-label={`移动 ${item.problem.title} 到文件夹`} value={item.folder} onChange={(e) => moveArchivedProblem(item.problem.id, e.target.value)}>{orderedFolders.map((folder) => <option key={folder} value={folder}>{"　".repeat(folder.split("/").length - 1)}{folderName(folder)}</option>)}</select><div className="row-actions"><button className="danger-action" onClick={() => deleteArchivedProblem(item)}>删除</button><i onClick={() => openArchivedProblem(item)}>进入 →</i></div></article>;
              })}
              {catalogItems.length === 0 && <div className={`catalog-empty ${!includeSubfolders && directChildFolders.length > 0 ? "has-subfolders" : ""}`}><b>{searchQuery ? "没有匹配的题目" : includeSubfolders ? "此文件夹及子文件夹暂无题目" : "当前层暂无题目"}</b><span>{searchQuery ? "请尝试其他编号或标题关键词。" : !includeSubfolders && directChildFolders.length > 0 ? "可从上方选择子文件夹继续浏览。" : "点击“添加题目”导入 JSON，或粘贴题面让 AI 自动生成。"}</span></div>}
            </div>
          </main>
        </div>
      </section> : <>
        ...
        <article className="problem-panel">
          ...
          {tab === "problem" ? (
            ...
            <div className="tests-content">
              <div className="tests-heading"><div><h2>测试点管理</h2><p>AI 会在一次请求中完成复杂度分析、边界覆盖与性能测试点生成。</p></div><div className="test-actions"><select aria-label="AI 生成测试点数量" value={testPointCount} onChange={(e) => setTestPointCount(Number(e.target.value))}><option value={12}>12 个</option><option value={18}>18 个</option><option value={24}>24 个</option></select><button className="ai-tests-button" disabled={generatingTests} onClick={generateMoreTests}>{generatingTests ? "正在快速生成…" : "✦ AI 生成测试点"}</button><button onClick={addTest}>＋ 手动添加</button></div></div>
