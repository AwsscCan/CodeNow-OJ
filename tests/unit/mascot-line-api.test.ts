...
describe("requestMascotLine", () => {
  it("未配置 API Key 时不发请求，直接用本地台词池", async () => {
    const line = await requestMascotLine({ ...baseReq, configured: false });
  ...
  it("成功时返回 AI 台词与表情", async () => {
    ...
    const line = await requestMascotLine({ ...baseReq, configured: true });
  ...
  it("请求携带 phase / 上下文 / recentLines", async () => {
    ...
    await requestMascotLine({ ...baseReq, recentLines: ["旧台词"], configured: true });
