...
export const ACWING_SOLVERS_1 = {
  AW785: { // 快速排序：输出排序后数列
    ...
    gen(rng) {
      ...
      const cases = [
        ...
        { input: mk(Array.from({ length: 10000 }, (_, i) => i + 1)), category: "adversarial", scale: 10000, targets: "已升序卡固定取首元素为枢轴", reason: "有序输入退化" },
        { input: mk(Array.from({ length: 10000 }, (_, i) => 100000 - i)), category: "adversarial", scale: 10000, targets: "严格降序卡固定枢轴", reason: "逆序输入退化" },
        { input: mk(randArray(rng, 15000, 1, 1000000000)), category: "performance", scale: 10000, targets: "满规模随机卡 O(n²) 排序", reason: "10 万随机数" },
        { input: mk(shuffle(rng, Array.from({ length: 10000 }, (_, i) => (i % 100) + 1))), category: "performance", scale: 10000, targets: "大量重复值的三路划分考验", reason: "每值约千次重复" },
      ];
  ...
  AW786: { // 第 k 个数
    ...
    gen(rng) {
      ...
      const big = randArray(rng, 15000, 1, 1000000000);
      const cases = [
        ...
        { input: mk(big, 1), category: "performance", scale: 10000, targets: "k=1 卡整体排序不必要的最坏枢轴", reason: "求最小值场景" },
        { input: mk(big, 100000), category: "performance", scale: 10000, targets: "k=n 满规模", reason: "求最大值场景" },
        { input: mk(big, 50000), category: "performance", scale: 10000, targets: "中位数快速选择 O(n) 期望", reason: "满规模取中" },
        { input: mk(shuffle(rng, Array.from({ length: 10000 }, () => 4)).concat([3]), 1), category: "adversarial", scale: 10000, targets: "近全等中藏唯一最小值", reason: "同值海洋找异类" },
      ];
  ...
  AW787: { // 归并排序：输出排序后数列
    ...
    gen(rng) {
      ...
      const cases = [
        ...
        { input: mk(Array.from({ length: 10000 }, (_, i) => 100000 - i)), category: "adversarial", scale: 10000, targets: "严格降序全量搬运", reason: "每层合并都交叉" },
        { input: mk(randArray(rng, 15000, 1, 1000000000)), category: "performance", scale: 10000, targets: "满规模随机 O(n log n) 吞吐", reason: "10 万元素" },
        { input: mk(Array.from({ length: 10000 }, (_, i) => i % 2 ? 2 : 1)), category: "performance", scale: 10000, targets: "奇数长度交错值", reason: "奇偶分割边界" },
      ];
  ...
  AW788: { // 逆序对的数量
    ...
    gen(rng) {
      ...
      const cases = [
        ...
        { input: mk(Array.from({ length: 10000 }, (_, i) => 100000 - i)), category: "performance", scale: 10000, targets: "全逆序答案约 5e9 卡 int 溢出与 O(n²)", reason: "n(n-1)/2 超 32 位" },
        { input: mk(randArray(rng, 15000, 1, 1000000000)), category: "performance", scale: 10000, targets: "满规模随机卡双重循环", reason: "10 万元素" },
        { input: mk(Array.from({ length: 10000 }, (_, i) => (i % 2 ? 1 : 1000000000))), category: "adversarial", scale: 10000, targets: "高低交错密集逆序", reason: "合并计数高频触发" },
      ];
  ...
  AW789: { // 数的范围：起止下标(0-based)，不存在 -1 -1
    ...
    gen(rng) {
      ...
      const big = randArray(rng, 20000, 1, 10000).sort((a, b) => a - b);
      const cases = [
        ...
        { input: mk(big, Array.from({ length: 10000 }, () => randInt(rng, 1, 10000))), category: "performance", scale: 10000, targets: "10 万×10 万卡线性扫描", reason: "必须二分" },
        { input: mk(big, Array.from({ length: 10000 }, () => 10001)), category: "adversarial", scale: 10000, targets: "全部未命中的边界越界", reason: "lower 落在 n 处" },
      ];
      for (let i = 0; i < 6; i++) {
  ...
  AW799: { // 最长连续不重复子序列(值 0~1e5)
    ...
    gen(rng) {
      ...
      const cases = [
        ...
        { input: mk(randArray(rng, 20000, 0, 100000)), category: "performance", scale: 10000, targets: "满规模随机卡 O(n²)", reason: "10 万元素" },
        { input: mk(Array.from({ length: 10000 }, (_, i) => i % 3)), category: "performance", scale: 10000, targets: "小值域高频收缩", reason: "窗口反复滑动" },
  ...
  AW800: { // 数组元素的目标和：升序 A,B 唯一解，输出 0-based i j
    ...
    gen(rng) {
      ...
      const bigA = asc(9000, 1, 20), bigB = asc(9000, 1, 20);
      const cases = [
        ...
        { input: mk(bigA, bigB, bigA[99999] + bigB[0]), category: "performance", scale: 10000, targets: "卡 O(n·m) 双重循环", reason: "解在 A 末端 B 前端" },
        { input: mk(bigA, bigB, bigA[0] + bigB[99999]), category: "performance", scale: 10000, targets: "双指针反向端点", reason: "解在 A 前端 B 末端" },
  ...
  AW801: { // 二进制中 1 的个数
    ...
    gen(rng) {
      ...
      const cases = [
        ...
        { input: mk(randArray(rng, 15000, 0, 2147483647)), category: "performance", scale: 10000, targets: "满规模卡逐位字符串转换低效", reason: "10 万数 lowbit 循环" },
        { input: mk(Array.from({ length: 10000 }, () => 2147483647)), category: "performance", scale: 10000, targets: "全高置位最坏循环次数", reason: "每数 31 次消位" },
  ...
  AW802: { // 区间和(离散化)
    ...
    gen(rng) {
      ...
      const bigAdds = Array.from({ length: 5000 }, () => [randInt(rng, -1000000000, 1000000000), randInt(rng, -10000, 10000)]);
      const bigQs = Array.from({ length: 5000 }, () => { const l = randInt(rng, -1000000000, 1000000000); return [l, randInt(rng, l, 1000000000)]; });
      const cases = [
        ...
        { input: mk(bigAdds, bigQs), category: "performance", scale: 10000, targets: "满规模卡 O(n·m) 逐点扫描", reason: "需离散化+前缀和" },
        { input: mk(bigAdds.slice(0, 50000), Array.from({ length: 5000 }, () => [999999999, 1000000000])), category: "adversarial", scale: 10000, targets: "询问集中在无数据的远端", reason: "二分越界处理" },
      ];
      for (let i = 0; i < 6; i++) {
  ...
  AW803: { // 区间合并：输出合并后的区间个数(端点相接算合并)
    ...
    gen(rng) {
      ...
      const bigDisjoint = Array.from({ length: 10000 }, (_, i) => [i * 10000 - 1000000000, i * 10000 - 1000000000 + 5]);
      const bigOverlap = Array.from({ length: 10000 }, () => { const l = randInt(rng, -1000000, 1000000); return [l, l + randInt(rng, 0, 2000000)]; });
      const cases = [
        ...
        { input: mk(shuffle(rng, [...bigDisjoint])), category: "performance", scale: 10000, targets: "10 万互不相交区间卡 O(n²) 两两比较", reason: "答案为 n" },
        { input: mk(shuffle(rng, [...bigOverlap])), category: "performance", scale: 10000, targets: "高重叠随机区间", reason: "大规模吞并链" },
